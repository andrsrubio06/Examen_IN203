...
unsigned char* inversePartialDiscretTransformFourier( sparseSpectralComposition const& sparse )
{
    ...
    std::fill(x, x+3*ni*nj, 0);
        ...
        for (std::uint32_t aux=0; aux<ni*nj;aux++ ){
            std::uint32_t n1=aux/ni;
            std::uint32_t n2=aux%ni;    
...
{
    std::chrono::time_point<std::chrono::system_clock> start, end_fourier, end_sparse, end_compressed_img;
    ...
    std::chrono::duration<double> temp_fourier = end_fourier-start;
    std::chrono::duration<double> temp_sparse = end_sparse-end_fourier;
    std::chrono::duration<double> temp_compressed = end_compressed_img-end_sparse;
    std::cout << "Temps calcul pour: "  << std::endl <<"Function de Fourier: "<< temp_fourier.count() 
                                        << std::endl <<"Function sparse: "<< temp_sparse.count()
                                        << std::endl <<"Function de compressing: "<< temp_compressed.count()
                                        << std::endl;
